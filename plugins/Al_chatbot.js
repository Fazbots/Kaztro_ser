...
if (conf.LANG == 'EN') {
    ...
    already_on = 'inrl chatbot is already on.'
    already_off = 'inrl chatbot is currently turned off!.'
    succ_on = 'inrl chatbot on! Restarting to make chatbot ✅'
    succ_off = 'inrl chatbot off Restarting to make normal  ❤️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ inrl chatbot സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    ...
    succ_off = 'inrl സെമി-ഫങ്ഷണൽ ആയി സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

New.addCommand({ pattern: 'inrl ?(.*)', desc: _dsc, fromMe: true,dontAddCommandList: true, usage: '.inrl on / off' }, (async (message, match) => {
