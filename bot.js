...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'inrlbot' || config.AFPLK == 'Fazbots' || config.AFPLK == 'vava' || config.AFPLK == 'inrlbot') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'Fazbots' || config.AFPLK !== 'Fazbots' || config.AFPLK !== 'vava' || config.AFPLK !== 'inrlbot') {
         console.log(
        ...
        console.log(
            chalk.green.bold('ɪɴʀʟʙᴏᴛ ᴡᴏʀᴋɪɴɢ ' + config.WORKTYPE + ' ɴᴏᴡ 💞'));
          // thanks to afnanplk
...
*/
                        ...
                        try {
                            ...
                            if (config.NOLOG === 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ ɪɴʀʟʙᴏᴛ ~______~*' +
